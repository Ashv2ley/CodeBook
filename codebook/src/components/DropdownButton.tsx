...
export default function DropdownButton(props: {placeholder: React.ReactNode; item1: React.ReactNode; item2: React.ReactNode; item3: React.ReactNode , item1ClickAction:any, item2ClickAction:any, item3ClickAction:any}) {
    ...
    return (
        <div className={isDarkMode ? 'dark' : ''}>
            <Select className={"flex"} placeholder={<div className="flex items-center">
                {props.placeholder}</div>}>
                <Option onClick={props.item1ClickAction} value={20} className={"flex items-center"}>{props.item1}</Option>
                <Option onClick={props.item2ClickAction} value={30} className={"flex items-center"}>{props.item2}</Option>
                <Option onClick={props.item3ClickAction} value={40} className={"flex items-center"}>{props.item3}</Option>
            </Select>
        </div>
    );
}
            ...
            slotProps={{
                ...
                root: (ownerState) => {
                    ...
                    return {
                        ...resolvedSlotProps,
                        className: clsx(
                            `relative text-sm font-sans box-border w-40 px-3 py-2 rounded-full text-left bg-white dark:bg-purple border border-solid border-slate-200 dark:border-neutral-700 text-slate-900 dark:text-neutral-300 transition-all hover:bg-slate-50 dark:hover:bg-purple/70 outline-0 ${
                ...
                listbox: (ownerState) => {
                    ...
                    return {
                        ...resolvedSlotProps,
                        className: clsx(
                            `text-sm font-sans p-1.5 my-3 w-40 rounded-xl overflow-auto outline-0 bg-white dark:bg-slate-900 border border-solid border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-300 [.open_&]:opacity-100 [.open_&]:scale-100 transition-[opacity,transform] [.closed_&]:opacity-0 [.closed_&]:scale-90 [.placement-top_&]:origin-bottom [.placement-bottom_&]:origin-top`,
                            resolvedSlotProps?.className,
                        ),
                    };
                },
